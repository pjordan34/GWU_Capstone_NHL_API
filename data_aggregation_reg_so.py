...
from event_filter import regular_season
...
# add in stats and fancy stats
    ...
    regDfcomplete['wshF'] = round((regDfcomplete['Goals_for'] + (0.2 * (regDfcomplete['Corsi_for'] - regDfcomplete['Goals_for']))), 2)
    regDfcomplete['wshA'] = round((regDfcomplete['Goals_A'] + (0.2 * (regDfcomplete['Corsi_A'] - regDfcomplete['Goals_A']))), 2)
    regDfcomplete['Goals_for%'] = round((regDfcomplete['Goals_for'] / (regDfcomplete['Goals_for'] + regDfcomplete['Goals_A'])) * 100, 2)
    regDfcomplete['Fenwick_for%'] = round((regDfcomplete['Fenwick_for'] / (regDfcomplete['Fenwick_for'] + regDfcomplete['Fenwick_A'])) * 100, 2)
    regDfcomplete['Corsi_for%'] = round((regDfcomplete['Corsi_for'] / (regDfcomplete['Corsi_for'] + regDfcomplete['Corsi_A'])) * 100, 2)
...
# write the new df out to csv
    regDfcomplete.to_csv('data/' + str(season) + 'agg_regular_season_so.csv')
    season = season + 1
